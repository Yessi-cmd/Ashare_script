...
将现有的行情提醒脚本升级为个人使用的量化研究与监控平台：策略可回测、信号可解释、数据可复用、工作日可通过浏览器安全访问，并能在普通个人服务器上稳定运行。网页侧支持多个独立账号，但监控告警仍以单一配置所有者为边界。

## 约束

- 监控告警保持单一配置所有者；网页允许少量受邀用户独立体验，不建设计费、多租户运营或复杂组织权限。
- 不自动下单，不连接券商交易接口。
